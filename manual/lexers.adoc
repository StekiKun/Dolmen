...
. The second limitation with `continue` is that it can be used solely
  ...
  Consider two rules which respectively recognize blocks `{ ... }`
  (resp. `[ ... ]`) delimited by curly brackets (resp. square
...
and the overalll number of all blocks that have been opened so far.
