...
public final class AutomataOutput {
	...
	private void genPattern(CSet cset) {
		buf.emit("// ").emitln(cset.toString());
		cset.forEach(c -> buf.emitln("case " + (int)c + ":"));
	}
	...
	private void genCell(int cellIdx, DFA.Cell cell) {
		buf.emit("private int ").emit(cellName(cellIdx))
			.emit("(").emit(")").openBlock();
		switch (cell.getKind()) {
		case PERFORM: {
			final Perform perform = (Perform) cell;
			buf.emitln("lastAction = " + perform.action + ";");
	...
	private void genEntry(Automata.Entry entry) {
		buf.emit("public Object ").emit(entry.name).emit("(");
		...
		buf.emitln("Arrays.fill(memory, -1);");
	 ...
	 * @throws IOException
	 */
	public static void output(FileWriter writer,
