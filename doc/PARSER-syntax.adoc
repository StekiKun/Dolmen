...
[[LSTRING]][[MLSTRING]]String literals (`MLSTRING`)::
...
[[ACTION]]Java actions (`ACTION`):: 
  ...
  associated to a parser rule's <<Production,productions>>, to express
  the Java return type of a <<Rule,rule>> or the Java type of the
  value associated to a <<TokenDecl,token>>, and also for the top-level
  ...
  Because Dolmen grammar rules can be <<FormalParams,parametric>>,
  ...
  any <<IDENT,identifier>> starting with a lower-case letter. When
...
[[ARGUMENTS]]Java arguments (`ARGUMENTS`):: 

  Java arguments are very similar to <<ACTION,Java actions>> in that
  they are lexical elements which represent verbatim excerpts of Java
  code. They are used to declare a <<Rule,parser rule>>'s arguments
...
*boldface*.  The main symbol is <<Parser,Parser>> and we use traditional
